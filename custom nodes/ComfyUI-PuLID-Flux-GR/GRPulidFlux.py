...
class GRApplyPulidFlux:
    ...
    def _cleanup(self, eva_clip, pulid_flux, flux_model):
        # IMPORTANT: Do not globally unload ComfyUI models here.
        # The previous implementation called UnloadAllModels.execute(), which
        # invokes model_management.unload_all_models() and can evict unrelated
        # models such as Qwen and Krea from memory between workflow stages.
        print("Performing PuLID local cleanup (global model unload disabled)...")
        torch.cuda.empty_cache()
        gc.collect()
        print("PuLID local cleanup completed.")
